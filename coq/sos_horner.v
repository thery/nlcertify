...
Variable R:Type.
Variable (rO rI : R) (radd rmul rsub rdiv: R->R->R) (ropp rsqr rsqrt rinv: R->R).
Variable (rneq req rle rge rlt rgt : R -> R -> Prop).
 ...
 Variable C : Type.
 Variable (cO cI: C) (cadd cmul csub cdiv : C->C->C) (copp cinv csquare : C->C).
 Variable (ceqb clt cle : C->C->bool).
 Variable phi : C -> R.
...
Lemma Pjump_tl P i l :
  P @ (tail (jump i l)) = P @ (jump i (tail l)).
Proof.
  apply env_morph. intros. rewrite <- jump_tl. f_equal.
Qed.
...
Section POWER.
 ...
 Section NORM_SUBST_REC.
  ...
  Fixpoint norm_aux (pe:PExpr) : Pol :=
   ...
   | PEadd pe1 (PEopp pe2) => Psub (norm_aux pe1) (norm_aux pe2)
...
Lemma norm_eval_spec pe1 pe2 : (norm_aux pe1 ?== norm_aux pe2) = true -> forall l, PEeval l pe1 == PEeval l pe2.
intros H l.
do 2 (rewrite norm_aux_spec).
...
(*
Lemma conj_PExpr_nonneg_concat2 l x y h1 h2 : PExpr_nonneg l x -> PExpr_nonneg l y ->  conj_PExpr_nonneg l h1 -> conj_PExpr_nonneg l h2 ->  conj_PExpr_nonneg l (x::y::h1 ++ h2).
Proof. simpl.
split. by [].
split. by [].
